	...
	public class StackTraceDrawer
	{
		...
		public void OnGUI(Rect drawRect, int clickCount)
		{
			...
			if(log != null)
			{
				...
				else if(clickCount == 1 && Event.current.button == 1 && Event.current.rawType == EventType.MouseUp && copyRect.Contains(Event.current.mousePosition))
				{
					Event.current.Use();
				...
				else if(clickCount == 2 && Event.current.button == 0 && Event.current.rawType == EventType.MouseUp && messageRect.Contains(Event.current.mousePosition))
				{
					Event.current.Use();
					clickCount = 0;					
