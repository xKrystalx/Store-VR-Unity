...
namespace Sisus.Debugging.Console
{
	...
	internal sealed class ConsoleWindowPlusExperimental : ConsoleWindowPlusBase, IHasCustomMenu, ISerializationCallbackReceiver
	{
		...
		private const float logEntryHeight = 38f;
		private const int DetailAreaPadding = 5;

		private const float resizerHeight = 5f;
		private static bool wasOpenedViaMenuItem = false;
		...
		[SerializeField]
		private List<Log> logs = new List<Log>(2500);
		...
		private static bool JustLaunchedEditor()
		{
			...
			const double MaxEditorStartupTime = 120d;
		...
		private void FetchInitialLogEntries()
		{
			...
			#else
			var messageField = logEntryType.GetField("condition", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			#endif
			...
			for(int i = 0; i < count; i++)
			{
				...
				{
					#if DEV_MODE
					UnityEngine.Debug.Log($"Exception? type={type}, file={file}\nmessage=\"{message}\"");
					#endif
			...
			#if DEV_MODE
			Debug.Log("Fetched initial posts");
			#endif
		}
		...
		private void OnApplicationStart()
		{
			...
			#if DEV_MODE
			Debug.Log("Cleared all during OnApplicationStart");
			#endif
		}
		...
		private void OnPlayModeStateChanged(PlayModeStateChange state)
		{
			if(state == PlayModeStateChange.ExitingEditMode)
			{
				...
				if(clearOnPlay)
				{
					...
					#if DEV_MODE
					Debug.Log("Cleared all posts because exiting edit mode");
					#endif
				}
			}
		...
		private void AssemblyCompilationStarted(string _)
		{
			#if DEV_MODE
			Debug.Log("Cleared all posts because AssemblyCompilationStarted");
			#endif
		...
		private void AssemblyCompilationFinished(string _, CompilerMessage[] _2)
		{
			ResubscribeToEvents();
		...
		protected override void OnGUI()
		{
			if(!initialized)
			{
				...
				#else
				toolbarSearchFieldStyle = GUI.skin.GetStyle("ToolbarSeachTextField");
				#endif
		...
		private void DrawMainList()
		{
			...
			if(!collapse)
			{
				for(i = firstVisible; i <= lastVisible; i++)
				{
					...
					GUI.color = GetLogGUIColor(log.type, log.AgeInSeconds, ref shouldRepaint);

					if(DrawBox(rect, icon, GetContent(log), boxStyle))
					{
		...
		private string GetContent(Log log)
		{
			if(!showTimestamps)
			{
				return log.text;
		...
		private GUIStyle GetMessageBoxStyle(bool isOdd, bool isSelected)
		{
			boxStyle.normal.background = isSelected ? boxBgSelected : isOdd ? boxBgOdd : boxBgEven;
			return boxStyle;
		}

		private Texture2D GetMessageIcon(LogType logType)
		{
			switch(logType)
			{
				case LogType.Error:
		...
		private Color GetLogGUIColor(LogType logType, double secondsShown, ref bool shouldRepaint)
		{
			if(!DebugLogExtensionsPreferences.LogUpdatedEffects)
			{
				return Color.white;
			}

			if(secondsShown >= 0.25d)
			{
				return Color.white;
			}
			...
			switch(logType)
			{
				...
				case LogType.Warning:
					...
					break;
				default:
					color = new Color(0.8f, 1f, 1f, 1f);
					break;
			}

			float t = (float)secondsShown * 4f;
			color.r = Mathf.Lerp(color.r, 1f, t);
			color.g = Mathf.Lerp(color.g, 1f, t);
			color.b = Mathf.Lerp(color.b, 1f, t);
		...
		private bool DrawBox(Rect rect, Texture2D icon, string content, GUIStyle boxStyle)
		{
			int firstLineEnd = content.IndexOf('\n');
			if(firstLineEnd != -1)
			{
				if(firstLineEnd == content.Length - 1)
				{
					content = content.Substring(0, content.Length - 1);
				}
				else
				{
					int secondLineEnd = content.IndexOf('\n', firstLineEnd + 1);
					if(secondLineEnd != -1)
					{
						content = content.Substring(0, secondLineEnd);
					}
				}
			}

			GUI.Label(rect, new GUIContent(content, icon), boxStyle);

			if(Event.current.type == EventType.MouseDown && rect.Contains(Event.current.mousePosition) && clickCount <= 1)
			{
				return true;
			}
			else if(clickCount == 2 && Event.current.type == EventType.MouseUp)
			{
				Event.current.Use();
				stackTraceDrawer.OpenScript();
				return false;
		...
		private void UpdateScrollToBottomSpeed(int newLogEntries)
		{
			...
			scrollToBottomSpeed = calculatedScrollDistanceToBottom * speedMultiplier;
		}
		...
		private void RebuildVisibleLogs(bool skipScrollAnimations)
		{
			...
			if(selectedLogWas != null)
			{
				...
				// TEMP TEST - THIS FIXES ISSUE WITH FILTER FIELD!
				//selectedVisibleLogIndex = visibleLogs.IndexOf(selectedLogWas);
			}
		}
