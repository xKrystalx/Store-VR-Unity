...
namespace Sisus.Debugging
{
	...
	public static class DebugLogExtensionsPreferences
	{
		...
		public static void Apply()
		{
			...
			Debug.formatter = new DebugFormatter(Formatting, MaxLengthBeforeLineSplitting, Colorize, NameValueSeparator, MultipleEntrySeparator, AllChannelsEnabledByDefault, colorTrueAsText, colorFalseAsText, colorStringAsText, colorNumericAsText, colorNameValueSeparatorAsText);
		...
		{
			var provider = new SettingsProvider("Preferences/Console", SettingsScope.User)
			{
				label = "Console",
		...
		#if !UNITY_2019_1_OR_NEWER
		[PreferenceItem("Console > User"), UsedImplicitly]
		#endif
		private static void DrawSettingsGUI()
		{
			...
			var setNameValueSeparator = EditorGUILayout.TextField("Name Value Separator", nameValueSeparator);
			...
			var setMultipleValueSeparator = EditorGUILayout.TextField("Multiple Entry Separator", multipleValueSeparator);
			...
			GUILayout.Label("Console+ (Experimental)", EditorStyles.boldLabel);

			bool scrollAnimations = ScrollAnimations;
			var setScrollAnimations = EditorGUILayout.Toggle("Scroll Animations", scrollAnimations);
			...
			var setHideDetailAreaWhenNoLogEntrySelected = EditorGUILayout.Toggle("Auto-Hide Detail Area", hideDetailAreaWhenNoLogEntrySelected);
			...
			var setLogUpdatedEffects = EditorGUILayout.Toggle("Log Updated Effects", logUpdatedEffects);
		...
		private static void DrawChannelsElement(List<string> channels, string prefsKey, Rect rect, int index)
		{
			...
			string setId = EditorGUI.TextField(rect, idWas);
